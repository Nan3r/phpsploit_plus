...
class Request:
    ...
    forwarder_template = {'POST': _load_template('forwarders/post.php')}
    ...
    def __init__(self):
        ...
        self.aespassword = hashlib.sha256(str(int(time.time())).encode('utf-8')).digest()
    ...
    def open(self, php_payload):
        ...
        #print('[*] php payload: %s' % php_payload)
        #if display_warnings(request):
    ...
    def Build(self, php_payload):
        ...
        php_payload = payload.Build(php_payload, self.parser, self.aespassword)
    ...
    def Read(self, response):
        ...
        try:
            response = payload.php2py(b_response, self.aespassword)
        except:
